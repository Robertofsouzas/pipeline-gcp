  ...
  tables=[
    {
        ...
        schema = file("C:/Users/ROBERTO/Desktop/Pipeline-stack-GCP/pipeline-gcp/bigquery/schema/gasolina_brasil/tb_historico_combustivel_brasil.json")
    }
  ]
}
...
module "bucket-raw" {
  ...
  name       = "data-pipeline-gcp1-combustiveis-brasil-raw"
  project_id = var.project_id
  location   = var.region
}
...
module "bucket-curated" {
  ...
  name       = "data-pipeline-gcp1-combustiveis-brasil-curated"
  project_id = var.project_id
  location   = var.region
}
...
module "bucket-pyspark-tmp" {
  ...
  name       = "data-pipeline-gcp1-combustiveis-brasil-pyspark-tmp"
  project_id = var.project_id
  location   = var.region
}
...
module "bucket-pyspark-code" {
  ...
  name       = "data-pipeline-gcp1-combustiveis-brasil-pyspark-code"
  project_id = var.project_id
  location   = var.region
}